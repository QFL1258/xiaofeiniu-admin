...
export default {
  ...
  methods:{
    ...
    makeQRCode(){
      //创建二维码的--注意此方法不能在当前组件的mounted中调用,因为绘图必须的canvas在el-dialogs中调用，对话框在组件加载时并不在DOM上
      ...
      //   })
      //把绘制得到的图片二进制数据转换为字符编码 转换为Base64编码的字符串
      qrcode.toDataURL(tableUrl,{
        width:300,
        errorCorrectionLevel:'H'
      },(err,url)=>{
        //console.log('二维码图片绘制而成,数据如下:')
        //console.log(url)
        this.qrcodeData=url;
      })
    }
